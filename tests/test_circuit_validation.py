...
def validate(s, bbp_check=False):
    return test_module.validate(s, bbp_check=bbp_check, print_errors=False)
...
def test_print_error(capsys):
    ...
    assert 'No "components" in config' in capsys.readouterr().out
...
def test_nodes_population_not_found_in_h5():
    with copy_circuit() as (circuit_copy_path, config_copy_path):
        ...
        assert errors == {
            Error(
                Error.FATAL,
                "populations not found in {}:\n{}".format(nodes_file, "\tfake_population\n"),
            )
        }
...
def test_no_required_node_single_population_datasets():
    with copy_circuit() as (circuit_copy_path, config_copy_path):
        ...
        assert errors == {
            Error(
                Error.FATAL,
                "Population default of {} misses datasets {}".format(nodes_file, ["node_type_id"]),
            )
        }
    ...
    for ds in required_datasets:
        with copy_circuit() as (circuit_copy_path, config_copy_path):
            ...
            assert errors == {
                Error(
                    Error.FATAL,
                    "Population default of {} misses datasets {}".format(nodes_file, [ds]),
                )
            }
...
def test_no_required_node_group_datasets():
    ...
    for ds in required_datasets:
        with copy_circuit() as (circuit_copy_path, config_copy_path):
            ...
            errors = validate(str(config_copy_path))
            assert errors == {
                Error(Error.FATAL, 'Group default/0 of {} misses "{}" field'.format(nodes_file, ds))
            }
...
def test_no_required_bio_node_group_datasets():
    ...
    with copy_circuit() as (circuit_copy_path, config_copy_path):
        ...
        assert errors == {
            Error(
                Error.FATAL,
                "Group default/0 of {} misses biophysical fields: {}".format(
                    nodes_file, required_datasets
                ),
            )
        }
...
def test_no_bio_component_dirs():
    ...
    for dir_ in dirs:
        with copy_circuit() as (_, config_copy_path):
            ...
            assert errors == {
                BbpError(Error.FATAL, 'Invalid components "{}": {}'.format(dir_, None))
            }
...
def test_no_edge_group_missing_requiered_datasets():
    with copy_circuit() as (circuit_copy_path, config_copy_path):
        ...
        assert errors == {
            Error(
                Error.FATAL,
                "Population default of {} misses datasets {}".format(edges_file, required_datasets),
            )
        }
...
def test_edge_population_missing_edge_group_index_one_group():
    with copy_circuit() as (circuit_copy_path, config_copy_path):
        ...
        assert errors == {
            Error(
                Error.FATAL,
                "Population default of {} misses dataset {}".format(
                    edges_file, {"edge_group_index"}
                ),
            )
        }
...
def test_edge_population_edge_group_different_length():
    with copy_circuit() as (circuit_copy_path, config_copy_path):
        ...
        assert errors == {
            Error(
                Error.FATAL,
                'Population {} of {} has different sizes of "group_id" and "group_index"'.format(
                    "/edges/default", edges_file
                ),
            )
        }
...
def test_edge_population_wrong_group_id():
    with copy_circuit() as (circuit_copy_path, config_copy_path):
        ...
        assert errors == {
            Error(
                Error.FATAL,
                "Population /edges/default of {} misses group(s): {}".format(edges_file, {1}),
            )
        }
...
def test_edge_population_wrong_group_index():
    with copy_circuit() as (circuit_copy_path, config_copy_path):
        ...
        assert errors == {
            Error(
                Error.FATAL,
                "Group default/0 in file {} should have ids up to {}".format(edges_file, 12),
            )
        }
...
def test_no_edge_source_to_target():
    with copy_circuit() as (circuit_copy_path, config_copy_path):
        ...
        errors = validate(str(config_copy_path))
        assert errors == {
            Error(Error.FATAL, 'No "source_to_target" in {}'.format(edges_file)),
